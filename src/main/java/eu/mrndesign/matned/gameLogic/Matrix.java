...
import eu.mrndesign.matned.fileOperations.Points;
import eu.mrndesign.matned.fileOperations.ScoreObject;
...
public class Matrix {
    ...
    private static final String PTS_TXT = "pts.";
    ...
    public Matrix(int col, int row, int numberOfBombs) {
        ...
        gameOverMessage = GAME_OVER_MSG;
        gameWinMessage = YOU_WIN_MSG;
        gameMessage = WELCOME_MSG;

        generateMatrix();
    }

    int getNumberOfClosedFields() {
        int temp = 0;
    ...
    public void openField(int x, int y) {
        ...
        if (matrix[x][y].isBomb()) {
            ...
            points = 0;
            gameMessage = gameOverMessage +" "+ points + PTS_TXT;
        ...
        if (getNumberOfClosedFields() == numberOfBombs) {
            ...
            this.gameTime = (int) ((System.nanoTime() - startTime)/timeFormatReducer);
            points = Points.get(gameTime,col,row,numberOfBombs);
            openAll();
            gameMessage = gameWinMessage+" "+ points + PTS_TXT;
            top100.read();
